# R script to prepare a download sample of URLs for images of Lepidoptera. For
...
# The cleaned GBIF metadata table created with the script lepidoptera-clean-gbif-urls.r
dt_clean_url <- readRDS("./data/gbif_cleaning/data_processed/dt_lepidoptera_eu_clean_url.rds")
...
dt_fam_stats_lic_ok
dt_fam_stats_lic_ok[, sum(n_sp_in_family)]
...
for (i in 1:nrow(dt_fam_stats_lic_ok)){
  ...
  seed <- 2021-10-21 # change date when you sample again
  set.seed(seed)
...
cbind(f_spl_dt[, .N, keyby = family],
      dt_fam_stats_lic_ok %>% arrange(family))

saveRDS(f_spl_dt, file = "./data/gbif_cleaning/data_processed/lepidoptera_sample_2021_10_21.rds")
